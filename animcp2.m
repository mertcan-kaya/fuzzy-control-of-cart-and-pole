...
elseif ~isempty(flag) & flag == 0,
    ...
    if ~flag,
        ...
        AnimCpAxisH = axes('unit', 'normal', 'pos', axisPos, 'visible', 'off');
        % ====== background frame
        ...
        cb1 = '';
        cb2 = [mfilename '([], [], [], [], ''target_pos'')'];

        string1 = 'Target Position:';
        string2 = 'Sinusoid Wave|Square Wave|Saw Wave|Mouse-Driven';
        ...
        tmp = [signalH -1 -1 -1 -1 -1 -1 -1 -1 -1];
        set(AnimCpFigH, 'userdata', [get(AnimCpFigH, 'userdata'); tmp]);
        drawnow % R2011a, avoid offset in screen
    ...
    %   refH = patch(real(ref), imag(ref), 'g');
        ...
        set(AnimCpAxisH, 'clim', [1 64], ...
            ...
            'box', 'on');
        axis equal;
        axis manual; % R2011a: to avoid axis change during simulation
        ...
        tmp = double([cartH poleH forceH refH]);
        tmp = reshape(tmp,[1 4]);
        tmp = [tmp -1 -1 -1 -1 -1 -1];
        set(AnimCpFigH, 'userdata', [get(AnimCpFigH, 'userdata'); tmp]);
...
%       set_param(winName, 'Simulation running', 'stop');
    ...
    elseif strcmp(action, 'target_pos'),
        ...
        if signal == 1, % sinusoid wave
            set_param(signal_block, 'waveform', 'sine');
            set_param(constant_block, 'value', '1');
        elseif signal == 2, % square wave
            set_param(signal_block, 'waveform', 'square');
            set_param(constant_block, 'value', '1');
        elseif signal == 3, % saw wave
            set_param(signal_block, 'waveform', 'sawtooth');
            set_param(constant_block, 'value', '1');
